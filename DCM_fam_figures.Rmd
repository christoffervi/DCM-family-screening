...
**Prevalence in men and women relatives**
```{r}
###############################################---------------------------------#
library(epiR);library(survival); library(tidyverse);library(ggtext)
#----------------------------------#
#Prevalence in men and women relatives- Evaluated at baseline
#---------------------------------#
fig2a <- 
  b %>%
  filter(Proband != "Y" &
...
spl <-
  ...
  geom_richtext(
    aes(
      x = 1,
      y = 35,
      label = "Age-standardized incidence **2.0**% per year <br>
                        <span style='font-size:11pt; color:gray'>**>250 x rate in general population**</span> <br>
                         <span style='font-size:11pt; color:gray'>*>15 x rate of HF in 55-59 year-olds*</span>"
    ),
    stat = "unique",
    angle = 30,
    show.legend = F,
